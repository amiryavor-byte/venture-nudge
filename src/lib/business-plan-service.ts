...
export interface RoadmapItem {
    ...
    category?: 'feature' | 'integration' | 'optimization' | 'research';
    effort?: 1 | 2 | 3 | 5 | 8;
    dependencies?: string[];
    assignee?: string;
    order?: number;
...
export interface Competitor {
    id: string;
    name: string; // Competitor name e.g. "LivePlan", "Enloop"
    marketPosition: 'leader' | 'challenger' | 'niche' | 'emerging';
    threatLevel: 1 | 2 | 3 | 4 | 5; // How serious a threat they are
    strengths: string; // What makes them strong
    weaknesses: string; // Where they're vulnerable
    targetMarket: string; // Who they serve
    ourAdvantage: string; // How we beat them
    website?: string; // Optional competitor URL
...
export interface MarketSegment {
    ...
    name: string;
    description: string;
    size: number; // TAM in dollars
    demographics: string;
    painPoints: string[];
    priority: 'primary' | 'secondary' | 'tertiary';
    accessibilityScore: 1 | 2 | 3 | 4 | 5;
    order: number;
}
...
export interface ValueProposition {
    ...
    benefit: string;
    evidence: string;
    segment: string; // Which market segment
    priority: 1 | 2 | 3 | 4 | 5;
    order: number;
}
...
export interface MarketingChannel {
    ...
    name: string;
    description: string;
    category: 'paid' | 'organic' | 'social' | 'content' | 'partnership' | 'events';
    budget: number;
...
export interface KeyMetric {
    ...
    name: string;
    description: string;
    category: 'growth' | 'financial' | 'operational' | 'customer' | 'product';
...
export interface RiskItem {
    ...
    name: string;
    description: string;
    category: 'market' | 'financial' | 'operational' | 'legal' | 'technical' | 'competitive';
    probability: 1 | 2 | 3 | 4 | 5; // Numeric 1-5 scale
    impact: 1 | 2 | 3 | 4 | 5; // Numeric 1-5 scale
    mitigation: string;
    mitigationStatus: 'none' | 'planned' | 'active' | 'resolved';
    order: number;
}
...
export interface RevenueStream {
    ...
    name: string;
    description: string;
    category: 'direct' | 'subscription' | 'advertising' | 'commission' | 'licensing' | 'other';
    pricingModel: string; // e.g., "Monthly subscription", "One-time purchase"
    averagePrice: number;
    expectedVolume: number;
    contributionPercent: number; // % of total revenue mix
    order: number;
}
...
export interface BusinessPlanData {
    ...
    // Legacy text fields (for migration)
